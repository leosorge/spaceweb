...
def mostra_intro_arcade():
    components.html("""
        ...
        (() => {
          ...
          async function startAudio(){if(playing)return;ac=new(window.AudioContext||window.webkitAudioContext)();await ac.resume();master=ac.createGain();master.gain.value=0.12;master.connect(ac.destination);playing=true;scheduleSequence(ac.currentTime);loopId=setInterval(()=>{if(playing)scheduleSequence(ac.currentTime);},LOOP*1000);document.getElementById("sw-audio-btn").innerText="🔊 Audio ON";}          function stopAudio(){playing=false;if(loopId){clearInterval(loopId);loopId=null;}if(ac){ac.close();ac=null;master=null;}document.getElementById("sw-audio-btn").innerText="▶ Audio arcade";}
...
# --- CICLO CORRETTO PER SCHERMATA_QUIZ IN APP.PY ---
        for i, (q_id, info) in enumerate(quiz_info.items()):
            with cols[i % 3]:
                st.markdown('<div class="card-anchor"></div>', unsafe_allow_html=True)
                logo_corso = info.get("logo", logo_default)
                nome_corso = str(info.get("nome", "")) # Recuperiamo il nome
                unita = "Qwat" if q_id == 2 else "Punti"
                
                # CONTROLLO DINAMICO SUL NOME (NON SULL'ID)
                if "coming soon" in nome_corso.lower() or nome_corso == "":
                    ...
                    if st.button(f"🚀 Avvia Quiz {q_id}", key=f"qt{q_id}", use_container_width=True):
                        ss.quiz_tipo = q_id
                        ss.quiz_idx = 0
                        ss.quiz_score = 0
                        ss.quiz_msg = ""
...
def schermata_gioco():
    ...
    with col_status:
        ...
        with col_dx:
            dx_sel = st.selectbox("ΔX", options=STEPS, format_func=lambda v: f"+{v}" if v>0 else str(v), key="sel_dx")
        with col_dy:
            dy_sel = st.selectbox("ΔY", options=STEPS, format_func=lambda v: f"+{v}" if v>0 else str(v), key="sel_dy")
        with col_go:
